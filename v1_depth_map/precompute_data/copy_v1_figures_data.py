...
def robust_copy2(src, target, retries=5, delay=10):
    ...
    for i in range(retries):
        try:
            ...
            if i < retries - 1:
                print(
                    f"  [RETRY {i+1}/{retries}] Failed copying file {src.name} due to {e}. Retrying in {delay}s..."
                )
                time.sleep(delay)
            else:
                raise
...
def robust_copytree(src, target, retries=5, delay=10, ignore=None):
    for i in range(retries):
        try:
            ...
            if i < retries - 1:
                print(
                    f"  [RETRY {i+1}/{retries}] Failed copying directory {src.name} due to {e}. Retrying in {delay}s..."
                )
                time.sleep(delay)
            else:
                raise
...
def copy_processed_session(flm_sess, session_name, dest_processed_dir):
    ...
    suite2p_ds_list = _normalise_ds_list(
        flz.get_datasets(
            origin_name=session_name,
    ...
    if suite2p_ds_list:
        no_ast = [
            ds
            for ds in suite2p_ds_list
            if not ds._extra_attributes.get("ast_neuropil", True)
        ]
        annotated = [
            ds for ds in suite2p_ds_list if ds._extra_attributes.get("annotated", False)
        ]
        if no_ast:
            suite2p_ds = no_ast[-1]
        elif annotated:
        ...
        if recordings is not None and not recordings.empty:
            for _, rec in recordings.iterrows():
                ...
                if isinstance(rec_traces, pd.DataFrame):
                    rec_traces_list = [
                        flz.Dataset.from_flexilims(id=ds.id, flexilims_session=flm_sess)
                        for _, ds in rec_traces.iterrows()
                    ]
                elif isinstance(rec_traces, pd.Series):
                    rec_traces_list = [
                        flz.Dataset.from_flexilims(id=ds.id, flexilims_session=flm_sess)
                        for ds in rec_traces
                    ]
    ...
    # 5. Eye tracking datasets (dlc_tracking, eye_reprojection)
    if session_entity is not None:
        if recordings is not None and not recordings.empty:
            for _, rec in recordings.iterrows():
                for ds_type in ["dlc_tracking", "eye_reprojection"]:
                    eye_ds = flz.get_datasets(
                        origin_name=rec["name"],
                    ...
                    if eye_ds is None:
                        continue
                    if isinstance(eye_ds, pd.DataFrame):
                        eye_ds_list = [
                            flz.Dataset.from_flexilims(
                                id=ds.id, flexilims_session=flm_sess
                            )
                            for _, ds in eye_ds.iterrows()
                        ]
                    elif isinstance(eye_ds, pd.Series):
                        eye_ds_list = [
                            flz.Dataset.from_flexilims(
                                id=ds.id, flexilims_session=flm_sess
                            )
                    ...
                    elif isinstance(eye_ds, list):
                        eye_ds_list = eye_ds
                    else:
                        eye_ds_list = [eye_ds]
                    for ds_obj in eye_ds_list:
                        if hasattr(ds_obj, "path_full") and ds_obj.path_full:
                            try:
                                rel = ds_obj.path_full.relative_to(processed_root_flz)
                                src_eye_path = src_processed_root / rel
                            except ValueError:
                                src_eye_path = ds_obj.path_full

                            # If it's a file, copy its parent directory to get everything
                            if src_eye_path.is_file():
                                paths_to_copy.add(src_eye_path.parent)
                            else:
                                paths_to_copy.add(src_eye_path)
            ...
            if hasattr(dataset_obj, "path_full"):
                ...
                if hasattr(dataset_obj, "path") and dataset_obj.path:
                    for s_root in [
                        ...
                        Path(
                            "/Volumes/lab-znamenskiyp/data/instruments/raw_data/projects"
                        ),
                    ]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Copy raw behavioral and processed session traces for V1 depth map figures to BlackPasspo."
    )
    parser.add_argument(
        "dest",
        type=str,
        ...
        nargs="?",
        help="Destination root directory for figures project.",
    )
    parser.add_argument(
        "--revisions-dest",
        type=str,
        default=None,
        help="Destination root directory for treadmill/revisions project. Defaults to the same as dest.",
    )
    parser.add_argument(
        "--src-project",
        type=str,
        default="hey2_3d-vision_foodres_20220101",
        help="Flexilims project name.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--offline",
        action="store_true",
        help="Run in offline mode using the local offline database JSON.",
    )
    ...
    flm_sess = flz.get_flexilims_session(
        project_id=args.src_project, offline_mode=args.offline
    )
    ...
    if db_src.exists():
        all_paths_to_copy.append(
            (db_src, dest_processed / "offline_database.json", "file")
        )
        if dest_rev != dest:
            all_paths_to_copy.append(
                (db_src, dest_rev_processed / "offline_database.json", "file")
            )
    else:
    ...
    for sess_name in tqdm(
        notebook_sessions, desc="Discovering figures", unit="session"
    ):
        try:
            # A. Processed copy
            processed_paths, s2p_suffix = copy_processed_session(
                flm_sess, sess_name, dest_processed
            )
            if processed_paths:
                for p in processed_paths:
                    ...
                    if src_p:
                        mode = "dir" if src_p.is_dir() else "file"
                        all_paths_to_copy.append((src_p, target, mode))

            # B. Raw metadata & video copy
            raw_paths = copy_raw_session(
                flm_sess, sess_name, dest_raw, ["SpheresPermTubeReward", "SizeControl"]
            )
            for p in raw_paths:
                src_p, target, _ = resolve_src_and_target(
                    p,
                ...
                if not src_p:
                    print(
                        f"WARNING: Path {p} is outside raw and processed roots. Skipping."
                    )
                    continue

                if src_p.is_file():
                    # Avoid copying heavy videos unless it's the exact eye tracking calibration session
                    if "video_file" in src_p.name or src_p.suffix in [
                        ".mp4",
                        ".avi",
                        ".tif",
                    ]:
                    ...
                    all_paths_to_copy.append(
                        (src_p, target, ("custom_dir", s2p_suffix))
                    )
    ...
    flm_rev = flz.get_flexilims_session(
        project_id="colasa_3d-vision_revisions", offline_mode=args.offline
    )
    ...
    for sess_name, protocol in tqdm(
        rev_sessions.items(), desc="Discovering revisions", unit="session"
    ):
        try:
            # A. Processed copy
            processed_paths, s2p_suffix = copy_processed_session(
                flm_rev, sess_name, dest_rev_processed
            )
            if processed_paths:
                for p in processed_paths:
                    src_p, target, _ = resolve_src_and_target(
                        p,
                        processed_root_rev_flz,
                        raw_root_rev_flz,
                        processed_root_rev,
                        raw_root_rev,
                        dest_rev_processed,
            ...
            raw_paths = copy_raw_session(
                ...
                sess_name,
                dest_rev_raw,
                ["SpheresTubeMotor", "SpheresPermTubeReward"],
            )
            for p in raw_paths:
                src_p, target, _ = resolve_src_and_target(
                    p,
    ...
    if args.dry_run:
        ...
        for src, target, mode in sorted(
            all_paths_to_copy, key=lambda x: (str(x[0]), str(x[1]))
        ):
    ...
    pbar = tqdm(
        sorted(all_paths_to_copy, key=lambda x: (str(x[0]), str(x[1]))),
        desc="Syncing files",
        unit="item",
    )
    for src, target, mode in pbar:
        ...
        if mode == "file":
            ...
            if s2p_suffix is not None:
                allowed_subs = [
                    f"suite2p_traces_{s2p_suffix}",
                    f"suite2p_traces_annotated_{s2p_suffix}",
                ]
            else:
            ...
            for sub in allowed_subs:
                ...
                if src_sub.exists():
                    if (
                        args.skip_existing
                        and target_sub.exists()
                        and any(target_sub.iterdir())
                    ):
                        continue
