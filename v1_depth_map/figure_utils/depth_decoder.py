...
def decoder_accuracy(
    ...
    if plot_type == "bar":
        ...
        for ndepths, color, marker, label in zip([5,8], 
            ...
            print(f"decoder accuracy {ndepths} depths: {len(decoder_results_ndepths)} sessions)")
...
def plot_closed_open_conmat(
    ...
    if vmax is None:
        vmax = np.round(np.max([conmat_closed, conmat_open]),2)  # round vmax to the closest multiple of 0.05
...
def calculate_error(conmat):
    ...
    if ndepths not in [5, 8]:
        ...
        mean_error = np.nansum(conmat.flatten() * errs) / np.nansum(
            conmat.flatten()
        )
        if ndepths == 5:
...
def plot_decoder_err_by_speeds(decoder_df, 
    ...
    for i, tick in enumerate(xticks):
        if i == 0:
            ...
            new_tick_labels.append(np.round(all_speed_bin_edges[:highest_bin][i-1]*100).astype('int'))
        else:
            new_tick_labels.append(f"> {np.round(all_speed_bin_edges[highest_bin-2]*100).astype('int')}")
