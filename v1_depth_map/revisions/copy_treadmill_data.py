...
def get_example_session_datasets(flm_sess, session_name):
    ...
    valid_recordings = recordings[recordings.protocol.str.contains("SpheresTubeMotor|SpheresPermTubeReward")]
...
def main():
    parser = argparse.ArgumentParser(description="Copy data used by treadmill.ipynb to a specific destination.")
    parser.add_argument("dest", type=str, help="Destination directory path.")
    parser.add_argument("--src-project", type=str, default="colasa_3d-vision_revisions", help="Source project ID in flexilims.")
    parser.add_argument("--src-raw-root", type=str, help="Override the source raw data root (e.g. if a volume is mounted at a non-standard path).")
    parser.add_argument("--src-processed-root", type=str, help="Override the source processed data root.")
    parser.add_argument("--dry-run", action="store_true", help="Print the paths that would be copied without copying.")
    parser.add_argument("--skip-existing", action="store_true", help="Skip files and directories that already exist in the destination.")
    ...
    src_processed_root = Path(args.src_processed_root) if args.src_processed_root else processed_root_flz
    ...
    if args.dry_run:
        ...
        for p in sorted(all_paths_to_copy):
            ...
            if args.src_processed_root:
                try: p = src_processed_root / p.relative_to(processed_root_flz)
                except ValueError: pass
            if args.src_raw_root:
                try: p = src_raw_root / p.relative_to(raw_root_flz)
                except ValueError: pass
    ...
    for p in sorted(all_paths_to_copy):
        ...
        if target_path.exists():
            if target_path.resolve() == p.resolve():
                print(f"Skipping {p.name} (source and destination are the exact same path)")
                continue
        ...
        if p.is_file():
            ...
            target_files = [
                "FrameLog.csv", "AllParams.csv", "NewParams.csv", "RewardLog.csv", 
                "RotaryEncoder.csv", "harpmessage.npz", "monitor_frames_df.pickle",
                "harpmessage.bin", "butt_camera_timestamps.csv", "face_camera_timestamps.csv",
            ...
            for sub in ["suite2p_traces_0", "suite2p_traces_annotated_0"]:
                ...
                if src_sub.exists():
                    ...
                    shutil.copytree(src_sub, target_path / sub, dirs_exist_ok=True, 
