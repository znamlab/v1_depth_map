...
def create_trajectory_animation(
    ...
    cbar_ax = g.fig.add_axes([0.85, 0.25, 0.01, 0.4])
    ...
    def animate(frame_id):
        ...
        nframes_per_trial = np.hstack(
            [[0], np.cumsum([len(trs) for trs in rs_by_trials])]
        )
    ...
    anim = animation.FuncAnimation(
        g.fig, animate, init_func=init, frames=frames, interval=50, blit=True
    )
