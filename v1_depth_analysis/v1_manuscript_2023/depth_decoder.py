...
def concatenate_all_decoder_results(flexilims_session, session_list, filename="decoder_results.pickle"):
    isess=0
    ...
    for session in session_list:
        ...
        if filepath.is_file():
            ...
            results.at[isess, "accuracy_closedloop"] = decoder_dict["accuracy_closedloop"]
...
def dot_plot(
    ...
    ax = fig.add_axes([plot_x, plot_y, plot_width, plot_height])
    ax.plot([0.3, 0.7], [group1, group2], f'{colors[0]}o-', alpha=0.7, label=labels[0], markersize=markersize, linewidth=linewidth)
    ax.axhline(y=baselines[0], color=colors[0], linestyle='dotted', linewidth=linewidth)
    t_statistic, p_value = ttest_rel(group1, group2)
    print(f"t-test for {labels[0]}: {p_value}")
    if (group3 is not None):
        ax.plot([0.3, 0.7], [group3, group4], f'{colors[1]}o-', alpha=0.7, label=labels[1], markersize=markersize, linewidth=linewidth)
        ax.axhline(y=baselines[1], color=colors[1], linestyle='dotted', linewidth=linewidth)
...
def calculate_average_confusion_matrix(results, conmat_column, sfx):
    results[f"conmat_prop{sfx}"] = results[f"{conmat_column}{sfx}"].apply(lambda x: x/np.sum(x))
    conmat_mean = np.mean(np.stack(results[f"conmat_prop{sfx}"]), axis=0)
    return results, conmat_mean

def plot_confusion_matrix(conmat,
                            ax,
                            vmax,
                            fontsize_dict,
                                  ):
...
def plot_closed_open_conmat(conmat_closed,
                            conmat_open,
                            normalize,
                            fig,
                            plot_x,
    ...
    ax = fig.add_axes([plot_x+plot_width/2, plot_y, plot_width/2*0.8, plot_height*0.8])
